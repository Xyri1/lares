...
多くのエージェントペットは、ライフサイクルイベントを決まったアニメーションに
対応づけるだけです。Lares はモデルに小さな一人称インターフェース——
`feel(valence, activation, control)`——を与え、現在の状況評価をキャラクターへ
投影できるようにします。その簡潔な報告を、Lares がローカルかつ決定論的に、
連続したキャラクター固有の演技へ変換します。

Lares は会話ログもモデル内部も読みません。感じ方はモデル自身が報告し、
フックは作業中や入力待ちといった運用上の事実だけを報告します。

`エージェントの状況評価 → feel(v, a, c) → ローカルのキャラクター演技 → Live2D`
...
取り込んだばかりのモデルも、キャリブレーション不要で即座に標準アンカーの
演技を行います。そのモデル自身の表情によりよく合わせたい場合は、マニフェスト
に `anchors` と `renderers.live2d.performance` の配線を手書きしてください——
アプリ内キャリブレーション機能は計画中ですが、まだ実装されていません。

対応範囲、`lares/1` マニフェスト、アンカーと配線の書き方、コマンドラインからの
