...
多数智能体桌宠只是把生命周期事件映射成固定动画。Lares 为模型提供一个极小的
第一人称接口——`feel(valence, activation, control)`——让它把当前的自身感受投射到
角色上。Lares 再在本地以确定性方式，将这份精简汇报转化为连续且贴合角色的表演。

Lares 不读取对话记录或模型内部状态。感受由模型自行汇报；钩子只汇报工作中、
等待输入等运行事实。

`智能体自身感受 → feel(v, a, c) → 本地角色表演 → Live2D`
...
新导入的模型无需任何校准，立刻就能演出内置的默认姿态锚点。如果想让它更贴合
自己的表情，可以在清单里手写 `anchors` 和 `renderers.live2d.performance`
接线——应用内校准工作流已在规划中，但尚未实现。

关于兼容范围、`lares/1` 清单格式、锚点与接线的写法和命令行导入流程，
