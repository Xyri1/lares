...
新角色导入后带有动作提示（cue），但还没有情感含义。托盘会显示当前状态：

| 托盘文字 | 含义 |
| --- | --- |
| 🔴 表情尚未映射 | 没有任何 cue 具备坐标 |
| 🟡 还剩 *n* 个表情 | 部分 cue 具备坐标 |
| 表情已映射 | 每个 cue 都具备坐标 |

未映射的 cue 仍然可用：你的 agent 可以按名字播放它。但情感引擎不会自行选中它。

要映射这些 cue，请选择 **映射表情…**。Lares 会把一段提示词复制到剪贴板。把它粘贴
到一个已连接 Lares MCP 的 agent 会话里。随后 agent 会在你的桌面上逐个预览 cue，
并在需要时向你提问。

整个流程中请保持 Lar 可见。映射是一个视觉判断，agent 需要借你的眼睛。

[角色包指南](character-format.md)包含清单文件结构、命令行导入流程和完整提示词。
...
| 工具 | 作用 |
| --- | --- |
| `emote` | 播放一个 cue，或直接驱动参数 |
| `list_cues` | 列出全部 cue，及其坐标和来源 |
| `status` | 报告当前角色和会话摘要 |
| `list_parameters` | 列出已加载模型的参数 |
| `preview_expression` | 精确预览某个表情，用于校准 |
| `save_expression` | 写入一个新创作的表情 |
| `update_expression` | 修改某个 cue 的坐标或参数 |

服务会告诉 agent 何时该表达情绪：会话开始、状态变化、连续第三次失败、恢复，以及
完成时。agent 不应为每次工具调用都表达情绪。
