...
| 菜单项 | 作用 |
| --- | --- |
| **角色** | 选择当前 Lar。该子菜单中还有禁用的**导入角色（即将推出）**和**打开角色目录** |
...
此版本暂不支持导入新的第三方模型。托盘菜单会显示禁用的
**导入角色（即将推出）**；Lares 目前加载内置的 Haru 角色。

[角色包指南](character-format.md)包含清单文件结构、通道表和供贡献者使用的命令行开发流程。
