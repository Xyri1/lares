...
5. 确认内置的 Haru 角色成功加载，且**导入角色（即将推出）**可见并处于禁用状态。
6. 更改缩放、勿扰、开机启动、位置和自动更新偏好。重启后确认每项设置都被保留。
...
10. 退出 Lares，然后运行 `/Applications/Lares.app/Contents/MacOS/Lares --uninstall`
    ...
    而托管数据、设置和位置仍保留在 Lares 的应用支持目录中。
...
结论：**通过 —— 维护者于 2026-08-07 确认。**

## Windows 全新机器验收 —— 2026-08-07 通过
...
2. 运行它。未签名构建预期会显示 **Windows 已保护你的电脑**；选择**更多信息 → 仍要
   ...
   制造一个虚假的通过结果而关闭系统级保护。
3. 重复 macOS 的第 4 至 9 步：托盘模式启动、内置角色与禁用的导入入口、持久化与重启、
