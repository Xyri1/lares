...
7. 检查只读的校准状态行：未完成校准的角色显示「表情映射 n/6 — 请运行
   Calibrate Lar」，全部映射完成后显示「表情映射 6/6」，且该行始终不可点击。
   在 harness 会话中运行 **Calibrate Lar** 技能，确认每映射一个 cue 托盘计数
   随之推进，且在 6/6 之前 `emote` 拒绝播放 cue。
8. 选择 **配置 Agent 集成…** 并取消一次；确认没有发生任何市场或插件变更。再选一次，
   ...
   入口发出 agent 情绪表达。如果这台机器曾运行过 009 之前的构建，还需确认首次启动已
