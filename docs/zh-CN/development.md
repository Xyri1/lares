...
7. **引擎推进。** `affect/engine.ts` 以 10 Hz 运行，施加衰减、心情平均、推动和饱和
   缩放。
8. **发出数据流。** 大脑通过 IPC 发送 `affect:update`。
9. **身体渲染。** `synth/` 计算逐帧数值，`runtime/` 把它们写入模型。
...
需要视觉检查时，从开发面板播放一个黄金场景。场景播放器走的是与真实流量相同的入口路径。
