...
The mapping `target = f(p, anchors)` is pure and memoryless. It projects nonzero `p` to the cube shell with Chebyshev magnitude, trilinearly blends the eight corner anchors there, then linearly blends from neutral by magnitude. It is exact at all nine anchors, convex, linear along every ray, and puts wire magnitude 1 exactly halfway to magnitude 2. Complete equations and required properties are normative in slice 013 SPEC §4.

Production evaluates the anchor map at a fixed `k = 1` (014-D5): wire magnitude is the only commitment signal, and no installed expressiveness configuration exists. A dev-only panel preview may evaluate another `k` without persisting it, changing the latch, or altering production playback. Every target or overlay change uses one fixed critically damped ease of about 700ms *(default)* from current channel values. When the displayed feel changes on a choreography-enabled Live2D body, one character-authored motion phrase additionally plays once — selected and modulated purely from the tuple (slice 014 SPEC §§4–6) — then physical ownership returns to the latched target. After settling, the target holds indefinitely while mechanical blink, breath, sway, physics, and the phrase's settled Part drawing continue. No decay, mood, cue selection, expression queue, baseline nudge, or hook-derived emotional beat exists (P4/P8).
...
Scenario file: `{ name, timeScale, events: [{ at_ms, envelope | feel }] }`. Player events use the same validated session and feel paths as real traffic, after HTTP-only origin checks. Golden scenarios ship in-repo: `brutal-debugging-session`, `smooth-build`, `long-wait-for-input`, and `recovery-arc`. The dev panel provides one playback stage with play/pause, deterministic seek, speed, semantic V/A/C and operational preview, a preview-only expressiveness `k` (production is fixed at `k = 1`, 014-D5), channel/wiring output, and live MCP → feel → feed → renderer tracing. Default Lar size for normative viewing is 400 logical px tall, DPI-scaled.

## 8. Interfaces (the P6 seam)

**Performance feed — the P6 seam (D31).** Brain→body is `{ feel: { valence, activation, control } | null, operational: BaselineState }`, emitted on change; `null` selects the neutral anchor. The body never receives raw ingress events, session identities, renderer parameters, asset references, or model prose. Character anchors and operational poses are renderer-neutral channel data loaded with the body; Live2D wiring — including the authored choreography map (slice 014 SPEC §3) — remains inside its renderer block. `authoring:preview` / `authoring:revert`, `scenario:*`, transactional character load, `stage:pointer`, and `body:inventory` remain auxiliary physical/runtime channels, not affect semantics.

**IRuntime (body-internal):** `load(modelPath, fallbackPhysics?, choreographed?)`, `prepareLoad(id, modelPath, fallbackPhysics?, choreographed?)`, `commitLoad(id)`, `rollbackLoad(id)`, `finalizeLoad(id)`, `cancelLoad(id)`, `parameters()`, `setParams(batch, weight?)`, `releaseParams(ids)`, `resetParams()`, `applyExpression(ref|params, weight, fadeMs)`, `playMotion(group, index?, priority)`, `playManagedMotion(plan)`, `cancelManagedMotion()`, `hitTest(x, y)`, `alphaAt(x, y)`, and `larSize()`. Managed playback hides every choreography mechanic — one-cycle enforcement, tempo/displacement modulation, face-override precedence, Part preservation and reset, watchdog and failure containment, settle — behind the seam (slice 014 SPEC §8). Transactional load keeps the prior body rollback-capable until finalization. pixi-live2d-display is the sole v1 implementation; nothing outside `runtime/` imports it.
