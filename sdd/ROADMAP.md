...
**Post-close protocol follow-up (2026-07-31):** slice `sdd/slices/011-interjection/` adds a third `emote` branch keyed on the reasoning interjections agents already emit (*wait, hmm, aha, oh no*), mapped through a fixed table to arousal nudges that enqueue nothing — hook baselines keep supplying valence — and rewrites D26's adoption copy from a milestone checklist into a standing disposition that also pulls the deferred MCP tools into context. Additive and optional: the M3a wire contract for `cue` and `params` is untouched, and the hooks-only floor still carries §9 with zero emotes. Proposed, not decided (D34).
