...
**Host-guidance follow-up (2026-08-01):** slice `sdd/slices/012-host-guidance/` records the first fresh adoption failure after tool exposure was verified and opens a second, host-level instruction vector (012-D1). MCP remains the canonical emote contract; concise plugin-delivered context will reinforce it without moving appraisal into hooks. `UserPromptSubmit.additionalContext` is the decided surface — a static helper-local reminder gated on local liveness and a hidden settings toggle (012-D2) — implemented 2026-08-01; the A/B moment-coverage gate (G1) remains open.
