...
**R2 — Live2D rendering.** Load Cubism models from user-selected disk locations; user-supplied model import is first-class. Default character: bundle Live2D's official sample models (Hiyori-class) under the Free Material License, including per-character terms, license texts, and copyright notices — mirroring the compliance pattern of Live2D's own public sample repositories (CubismWebSamples ships Haru/Hiyori/Mao/Mark/Natori/Rice/Wanko in-repo). VTube Studio's Akari is excluded (test-stream-only, non-commercial terms). Final per-character terms read is an open task before release.
...
1. Final read of Free Material License per-character terms for the bundled sample models (R2), including required copyright-notice format.
2. Cubism Core distribution mechanics (user-loaded vs bundled per Live2D's redistribution terms).
3. Emote tag vocabulary v1 (proposal to be made in SPEC).
4. Cubism 5 runtime coverage — pixi-live2d-display covers Cubism 2.1/4; official Live2D Web SDK migration reserved for when third-party Cubism 5 characters arrive.
