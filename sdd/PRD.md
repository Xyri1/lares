...
**R2 — Live2D rendering.** The current release loads the redistribution-cleared, build-selected default character (voice-free Haru). User-supplied model import remains a first-class product goal but is deferred; its tray action stays visible and disabled until the workflow is release-ready. Additional samples require their per-character terms read before bundling. VTube Studio's Akari is excluded (test-stream-only, non-commercial terms). Character clearances are recorded in `sdd/clearances/M0-clearances.md`.
...
**R7 — Character package format.** Published schema, package validation through the repo dev script, and one fully worked example. In-app validation returns with third-party import. The format document is a first-class deliverable, not internal documentation.
...
**R9 — Minimal settings.** Character selection, a disabled third-party import affordance, size (Lar scale), do-not-disturb (hides the Lar; the daemon keeps tracking and mood survives the hide), position reset, launch-at-login, quit. Nothing else.
**R10 — Expression import & authoring.** Deferred with third-party model import. When restored, auto-import a model's bundled expressions and motions at character setup and provide a user-invoked authoring workflow.
